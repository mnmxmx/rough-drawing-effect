...
void main(){
  ...
  float edgeUvOffset = snoise3D(vec3(st * 10.0, time));

  vec2 faceColorUv = vUv + vec2(faceUvOffset)* 0.003;
  vec2 edgeColorUv = vUv + vec2(edgeUvOffset)* 0.0022;

  vec4 faceColor = texture2D(uTex_1, faceColorUv);
  vec4 edgeColor = texture2D(uTex_2, edgeColorUv);
  edgeColor.a = edgeColor.r;
  edgeColor.rgb = uEdgeColor;

  vec4 color;
  color.a = min(1.0, faceColor.a + edgeColor.a);

  

  color.rgb = mix(faceColor.rgb, edgeColor.rgb, edgeColor.a);
